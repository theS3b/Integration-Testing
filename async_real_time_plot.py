''' This file contains a function to plot data in real time in a thread. '''
...
def async_real_time_plot(title, x_label, y_labels, configuration, data: queue.Queue = None):
    '''
    This function can be used to plot data in real time.
    To use it, simply call it in a thread and put data in the queue.
    ...
    To stop the plotting, put (False, None) in the queue.
    ...
    while True:
        ...
        # Check for new data
        continue_run, new_data = data_queue.get(block=True)
        if not continue_run:
            break
